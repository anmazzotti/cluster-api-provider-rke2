...
var _ = Describe("Workload cluster creation", func() {
	...
	Context("Creating a single control-plane cluster", func() {
		It("Should create a cluster with 3 control plane node", func() {
			...
			Eventually(func() error {
				...
				for _, machine := range machineList.Items {
					if machine.Spec.NodeDrainTimeout != nil && machine.Spec.NodeDrainTimeout.Duration != duration240s.Duration {
						return fmt.Errorf("In place updated NodeDrainTimeout value is not propagated to Machine %s/%s", machine.Namespace, machine.Name)
					}
					if machine.Spec.NodeDeletionTimeout != nil && machine.Spec.NodeDeletionTimeout.Duration != duration240s.Duration {
						return fmt.Errorf("In place updated NodeDeletionTimeout value is not propagated to Machine %s/%s", machine.Namespace, machine.Name)
					}
					if machine.Spec.NodeVolumeDetachTimeout != nil && machine.Spec.NodeVolumeDetachTimeout.Duration != duration480s.Duration {
						return fmt.Errorf("In place updated NodeVolumeDetachTimeout value is not propagated to Machine %s/%s", machine.Namespace, machine.Name)
					}
				}
				return nil
